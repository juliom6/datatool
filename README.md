...
```bash
mkdir git && mkdir datatool_logs && cd git && git clone https://github.com/juliom6/datatool.git && cd datatool && sudo apt update && sudo apt install python3-pip -y && sudo apt install python3-venv -y && python3 -m venv .venv && source .venv/bin/activate
...
```
Then, create a runtime for the cluster
```bash
